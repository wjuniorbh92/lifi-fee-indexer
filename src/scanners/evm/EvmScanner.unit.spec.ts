...
const mockLogs = [
  {
    ...
    transactionHash:
      '0x13f791d14a9286d2503df5112f8b5cd84f5c06eaf0183ca59342c3a2f8f08f9b',
    logIndex: 0,
    args: {
  ...
  },
  {
    blockNumber: 84797174n,
    transactionHash:
      '0x6dfce7f2579281d91bba4c7fde5418da7e102ec761e5a0fd5210c23cd5480211',
    logIndex: 1,
    args: {
  ...
  },
  {
    ...
    transactionHash:
      '0x003978ac0b540ee6ad6d7cd5dd5c713b7b38cb9add9d7ff6c895305870e0b3f3',
    logIndex: 0,
    args: {
...
describe('EvmScanner', () => {
  ...
  describe('getEvents', () => {
    ...
    it('deduplicates block timestamp fetches', async () => {
      ...
      const getBlockCalls = (
        client as unknown as { getBlock: { mock: { calls: unknown[] } } }
      ).getBlock.mock.calls;
    ...
    it('passes correct block range to getLogs', async () => {
      ...
      expect(
        (client as unknown as { getLogs: { mock: { calls: unknown[][] } } })
          .getLogs.mock.calls[0][0],
      ).toMatchObject({
    ...
    it('throws RangeError when from > to', async () => {
      ...
      await expect(scanner.getEvents(100, 50)).rejects.toThrow(
        'Invalid block range',
      );
    });
    ...
    it('propagates RPC errors from getLogs', async () => {
      const client = createMockClient();
      (
        client as unknown as { getLogs: ReturnType<typeof vi.fn> }
      ).getLogs.mockRejectedValue(new Error('rate limited'));
      const scanner = new EvmScanner(mockConfig, client);

      await expect(scanner.getEvents(78_600_000, 78_602_000)).rejects.toThrow(
        'rate limited',
      );
    });
