...
const ORACLE_CONTRACT = 'CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC';
...
describe('StellarScanner E2E (Stellar testnet)', () => {
	...
	it('getEvents fetches real fee events from recent ledgers', async () => {
		...
		const to = latest;
		const result = (await scanner.getEvents(from, to)) as ScanBatchResultWithCursor;
	...
	it('cursor-based pagination continues from previous position', async () => {
		...
		// First fetch
		const result1 = (await scanner.getEvents(from, to)) as ScanBatchResultWithCursor;

		if (result1.nextCursor) {
			// Set cursor and fetch next window
			scanner.setCursor(result1.nextCursor);
			const result2 = (await scanner.getEvents(to + 1, to + 30)) as ScanBatchResultWithCursor;

			// Should not duplicate events from first fetch
			if (result1.events.length > 0 && result2.events.length > 0) {
				const ids1 = new Set(result1.events.map((e) => e.transactionHash + e.logIndex));
