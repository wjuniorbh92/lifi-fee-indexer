...
describe('sendError', () => {
	it('sends error with code and message', () => {
		...
		sendError(reply as never, 400, 'Invalid input', ApiErrorCode.VALIDATION_ERROR);
	...
	it('includes details when provided', () => {
		const reply = createMockReply();
		sendError(reply as never, 502, 'RPC failed', ApiErrorCode.RPC_FETCH_FAILED, { retries: 3 });
